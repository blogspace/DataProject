...
/**
  * @function
  * @author create by liuhao at 2019/7/10 21:15
  */
object HdfsUtils {
  ...
  def deleteFile(path: String) = {
    val conf = new hadoop.conf.Configuration()
    val fs = FileSystem.get(URI.create(path), conf)
    if (HdfsUtils.isFile(fs, path) == true) {
      //HdfsUtils.deleteFile(fs, path)
      HdfsUtils.createFile(fs, path)
    } else {
      HdfsUtils.createFile(fs, path)
    }
  }

  def appendToFile(content: String, path: String): Unit = {
    val conf = new hadoop.conf.Configuration()
