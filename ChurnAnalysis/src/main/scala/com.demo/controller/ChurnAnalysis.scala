...
/**
  * @function
  * @author create by liuhao at 2019/6/21 15:23
  */
object ChurnAnalysis {
  ...
  def main(args: Array[String]) {
    ...
    val train: Dataset[Account] = spark.read.option("header", "true")
      .schema(schema).csv(Constant.trainingData).as[Account]
    train.take(1)
    train.cache
    println(train.count)

    val test: Dataset[Account] = spark.read.option("header", "true")
      .schema(schema).csv(Constant.testData).as[Account]
    test.take(2)
    println(test.count)
    test.cache
